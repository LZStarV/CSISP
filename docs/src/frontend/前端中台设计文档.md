...
vi.mock('@/services/user', () => ({
  userService: {
    getUserList: vi.fn().mockResolvedValue({
      data: [{ id: 1, username: 'student001', realName: '张三' }],
      pagination: { total: 1 },
    }),
  },
}));
...
- [CSISP业务文档](/src/business/业务文档)
- [CSISP总体架构设计文档](/src/architecture/总体架构设计文档)
- [CSISP技术架构设计文档](/src/architecture/技术架构设计文档)
- [CSISP后端设计文档](/src/backend/后端设计文档)
- [CSISP数据库设计文档](/src/database/数据库设计文档)
...
- `Dashboard` 仪表盘：接入后端统计与图表（用户增长、课程分布、最近活动）
- `DataOverview` 数据总览：用户/课程/考勤/作业分页列表与筛选
- `UserManagement` 用户管理：分页查询、创建/编辑、删除、状态切换
- `CourseManagement` 课程管理：分页查询、创建/编辑、占位模块（教师/班级/课程安排）
- `AttendanceManagement`、`HomeworkManagement`、`NotificationManagement`：列表占位与后续对接
- `System/RoleManagement`、`System/PermissionManagement`：开发中占位页，说明后续功能
- `Profile`、`Password`：个人中心与修改密码占位页，说明开发中

路由结构调整：父路由 `/system` 使用容器组件，子路由使用相对路径 `roles`、`permissions`，避免嵌套路由渲染问题。

### 4.x 请求层与代理

- 前端统一以 `'/api'` 为 `baseURL`，通过 Vite 代理转发到后端（开发环境）：
  - `vite.config.ts:server.proxy` 将 `'/api'` 指向 `http://localhost:3000`
- 认证接口路径对齐后端：
  - 登录：`POST /api/users/login`
  - 当前用户：`GET /api/users/me`（需 `Authorization: Bearer <token>`）
- 响应拦截器策略：
  - 401 仅清理本地 `token`，不强制跳转；由页面逻辑决定后续行为
  - 统一接受 `2xx` 业务码，非 `2xx` 以错误提示呈现

### 4.x 表格组件与分页规范

- Naive-UI `n-data-table` 的 `row-key` 在当前版本需传入函数；统一封装在中台表格组件：
  - `BaseTable` 内部将字符串 `rowKey` 解析为函数 `(row) => row[id]`
- 服务端驱动分页：
  - 传入 `itemCount`（总条数），绑定事件 `onUpdatePage`、`onUpdatePageSize`
  - 查询时携带 `page/size` 参数，收到返回后更新 `current/pageSize/total`

### 4.x 搜索组件交互

- `BaseSearch` 通过 `update:model` 事件向父组件同步表单值；页面必须监听并合并：
  - `@update:model="onUpdateSearchModel"` → `Object.assign(searchModel, m)`
- 支持 `input/select/date/daterange` 等类型，重置时清空并回调 `reset`

### 4.x 图标与响应式

- 避免将组件对象放入响应式数据结构造成性能警告：
  - 对 `@vicons/ionicons5` 组件在卡片配置中使用 `markRaw(IconComponent)`
