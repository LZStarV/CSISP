...
- `supabase/`：数据库迁移与配置（Supabase CLI 管理的唯一事实来源）
- `docs/*`：文档站点（VitePress）
...
- `backend-integrated/`
  ...
  - `src/infra/`：`supabase/*`（通过 supabase-sdk 访问）、`redis/*`、`mongo/*` 客户端与封装
...
- `packages/supabase-sdk`：Nest 封装的 supabase-js 客户端与数据访问门面
- `infra/idl`：
...
- 根脚本（摘录）：
  - `supabase:link:stag` / `supabase:pull:stag`：Supabase 本地链路（link/pull）
  - `idl:gen`：生成 Thrift 类型
  - `dev:backend-integrated`：启动后端开发服务（已接入 supabase-sdk）
  - `dev:backoffice`：已移除（Backoffice 待开发）
  - `dev:bff`：启动 Koa 聚合层
