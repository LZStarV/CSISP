...
| ------------------------------ | ---------------------------------------- | -------------- | ---------------------------------------------------------------- | ------------------------- |
| `PostgreSQL`（Supabase）       | supabase、backend-integrated、backoffice | 关系型数据库   | 迁移统一托管；RLS 与 SECURITY DEFINER RPC                        | Supabase CLI 最新         |
| `supabase-js` + `supabase-sdk` | backend-integrated、backoffice           | 数据访问门面   | 统一 RLS 用户态读取与服务端 RPC 写入                             | supabase-js^2.x           |
| `Redis`（Upstash）             | bff、backend-integrated                  | 缓存与限流     | 统一通过 `@csisp/redis-sdk` 接入 Upstash；固定窗口限流用 INCR+EX | 见模块                    |
| `Redis`（`ioredis`）           | backoffice                               | 缓存/会话等    | 后台系统保留 ioredis（后续视情况迁移）                           | 见模块                    |
...
- Redis 接入：bff 与 backend-integrated 通过 `@csisp/redis-sdk` 统一接入 Upstash，并在拦截器/中间件层实现固定窗口限流（INCR+EX）；backoffice 保留 `ioredis`，后续视情况迁移
