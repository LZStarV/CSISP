...
- **日志**: 结构化日志（process.stdout/stderr，由 Nest 拦截器统一输出）
- **异常处理**: Nest 全局异常过滤器

### BFF 技术栈

- **应用**: `apps/bff`（Koa 2.x + TypeScript）
- **框架**: Koa 2.x
- **路由**: `@koa/router`
- **请求解析**: `koa-bodyparser`
- **HTTP 客户端**: `undici`（通过 `@csisp/upstream` 封装）
- **配置**: `dotenv`
- **构建 / 运行**: tsx + esbuild
...
│   ├── 📁 middlewares/               # 通用中间件（Koa / 其他）
│   ├── 📁 upstream/                  # 上游 HTTP 客户端封装
│   ├── 📁 utils/                     # 工具函数
│   └── 📁 validation/                # 参数校验与 schema
├── 📁 docs/                          # 文档层 - 项目文档和指南
...
> 所有共享能力通过 `packages/*` 暴露，供 backend-integrated、BFF 与前端复用。

```
packages/
├── 📁 types/         # @csisp/types：共享业务类型与 API/DTO 定义
├── 📁 redis/         # @csisp/redis：Redis 客户端封装
├── 📁 middlewares/   # @csisp/middlewares：通用中间件实现
├── 📁 upstream/      # @csisp/upstream：HTTP 上游客户端封装
├── 📁 utils/         # @csisp/utils：工具函数
├── 📁 validation/    # @csisp/validation：参数校验与 schema
