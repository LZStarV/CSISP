...
│   └── 📁 db-workflows/              # 数据库工作流（Postgres 迁移/种子、Mongo 种子）
├── 📁 docs/                          # 文档层 - 项目文档和指南
...
- Development：本地通过 `infra/database/docker-compose.db.yml` 启动 `postgres + redis + mongo`
...
└── 📁 db-workflows/  # @csisp/db-workflows：数据库迁移与种子统一入口
```
...
sequenceDiagram
  ...
  Main->>Main: 加载根 .env 与 apps/backend-integrated/.env
