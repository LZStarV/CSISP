...
> 说明：后端现为“环境变量优先”，默认端口为 5433（宿主机映射至容器 5432）；当在容器网络内访问时可使用服务名与容器内端口（如 `postgres:5432`）。
...
db-schema 包内包含 `/.sequelizerc` 与 `sequelize-config.cjs`，用于兼容 `sequelize-cli` 的加载方式（CLI 以 CommonJS 读取配置）；后端不再直接持有迁移与种子目录。
...
```mermaid
erDiagram
  COURSE ||--o{ SUB_COURSE : has
  SUB_COURSE ||--o{ TIME_SLOT : has
  COURSE {
    int id
    string course_name
    string course_code
...
erDiagram
  ...
  HOMEWORK_SUBMISSION ||--o{ HOMEWORK_FILE : has
  HOMEWORK {
    int id
    int class_id
    string title
