...
- 停止旧栈：`docker compose down`
- 或调整 `infra/database/.env.db` 中的 `DB_PORT_HOST`/`REDIS_PORT_HOST`
...
推荐（新）使用 `infra/database/scripts/init_*.sh/.bat` 一键启动与初始化：
...
环境变量（可从 `infra/database/.env.db` 注入）：`DB_HOST`、`DB_PORT`、`DB_USER`、`DB_PASSWORD`、`DB_NAME`。
