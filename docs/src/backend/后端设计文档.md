...
- 运行时装配入口：`apps/backend/src/databases/index.ts`
- 连接来源：`apps/backend/src/database.ts:26-33` 提供单一 `sequelize` 实例与 `pool/logging/benchmark`
- 模型来源：动态加载 `apps/backend/sequelize/models/*.ts` 工厂（不复制模型），再执行各模型的 `associate(models)`
- 控制器装配：`apps/backend/src/initControllers.ts:25-31` 等待 `modelsReady` 后注册到 `ServiceFactory`
- 服务层使用：`apps/backend/src/services/BaseService.ts:19,62,91,120` 统一 CRUD；具体业务如 `apps/backend/src/services/UserService.ts` 进行字段映射与关联查询

```mermaid
sequenceDiagram
  participant App as app.ts
  participant Boot as initializeControllers()
  participant DB as src/databases/index.ts
  participant Loader as loadModels()
  participant Assoc as setupAssociations()
  ...
  App->>Boot: initializeControllers()
  Boot->>DB: import models, modelsReady
  DB->>Loader: loadModels(sequelize)
  Loader-->>DB: Models(static)
  DB->>Assoc: associate(models)
  Assoc-->>DB: Models(ready)
  Boot->>Factory: registerModels(models)
  Factory-->>Ctrl: construct controllers
  Ctrl->>Svc: call service methods
  Svc->>ORM: create/find/update/destroy
  ORM-->>PG: SQL
  PG-->>ORM: rows/result
  ORM-->>Svc: instances/data
  Svc-->>Ctrl: ApiResponse
  Ctrl-->>App: Router response
```
  ...
  "data": [
    {
      ...
      "type": "attendance",
      "title": "学生打卡",
      "description": "状态：normal",
      "timestamp": "2025-01-15T09:05:00Z",
      "user": { "id": 2, "realName": "张三" }
    },
    {
      "id": 2002,
      "type": "homework",
...
- 领域类型来源：`packages/types/src/index.ts:4-22`
- ORM 细化类型：`apps/backend/src/types/models.ts:403-428` 定义 `Models/DatabaseInstance` 及各 `Attributes/CreationAttributes`
- 服务层映射示例：
  - 写入用户：`apps/backend/src/services/UserService.ts:52-61` 使用 `student_id ← studentId`、`real_name ← realName`、`enrollment_year ← enrollmentYear`
  - 统计查询：`apps/backend/src/services/UserService.ts:299-305` 按 `enrollment_year` 分组

```mermaid
flowchart TD
  A[Request DTO (@csisp/types)] --> B[Controller 参数校验]
  B --> C[Service 字段映射 camelCase→snake_case]
  C --> D[Sequelize Model 层]
  D --> E[(PostgreSQL 表)]
  E --> F[Model 实例]
  F --> G[Service 组装响应]
  G --> H[Controller 返回 ApiResponse]
```

### 5.4 启动命令与中间件链路

- 启动命令
  - 根目录：`pnpm --filter backend dev`
  - 后端目录：`pnpm dev`（`apps/backend/package.json:7`）
- 中间件链路（按注册顺序）
  - `error`：统一异常捕获与响应
  - `cors`：跨域策略
  - `logger`：请求日志
  - `rateLimit`：限流防护
  - `bodyParser`：解析 JSON/表单
  - `auth`：JWT 校验与 `ctx.state.user`
  - `router`：`RouterConfig.getRouter()` 挂载 `/api`
...
```typescript
// 使用 @csisp/redis 封装的客户端
import { get, set, del } from '@csisp/redis';

// 课程详情缓存示例
async function getCourseDetailWithCache(courseId: number) {
  ...
  // 2. 未命中时查询数据库
  const course = await CourseModel.findByPk(courseId /* include 省略 */);
  if (!course) return null;

  // 3. 写入缓存，设置合理 TTL（例如 300 秒）
  if (process.env.REDIS_ENABLED === 'true') {
    await set(cacheKey, JSON.stringify(course), 300);
  }
...
const CacheKeys = {
  // 用户相关
  user: (id: number) => `be:user:${id}`,
  userPermissions: (id: number) => `be:user:${id}:permissions`,
  userCourses: (id: number) => `be:user:${id}:courses`,

  // 课程相关
  course: (id: number) => `be:course:${id}`,
  courseDetail: (id: number) => `be:course:detail:${id}`,
  courseStudents: (id: number) => `be:course:${id}:students`,
  courseTeachers: (id: number) => `be:course:${id}:teachers`,
...
const gatewayRoutes = {
  ...
  '/api/dashboard/*': 'http://dashboard-service:3006',
};
...
> 说明：生产环境推荐将数据库与 Redis 放入 `infra/database` 独立 compose；后端容器可与其叠加运行，通过服务名访问 `postgres`/`redis`（如 `DB_HOST=postgres`）。下列示例用于说明整体编排结构。
...
// 数据库健康接口（直连校验）
router.get('/api/db/health', async ctx => {
  try {
    await sequelize.authenticate();
    ctx.status = 200;
    ctx.body = { code: 200, message: '数据库连接正常' };
  } catch (e: any) {
    ctx.status = 503;
    ctx.body = { code: 503, message: '数据库连接失败', error: e?.message };
  }
});
...
sequenceDiagram
  ...
  App->>DB: import models, modelsReady
  App->>DB: await modelsReady (加载模型 + 执行associate)
  ...
  App->>App: 挂载中间件（error/cors/logger/rateLimit/bodyParser/auth）
...
flowchart TD
  ...
  C --> D[写入 AttendanceRecord(task_id,user_id)]
  D --> E{状态统计}
