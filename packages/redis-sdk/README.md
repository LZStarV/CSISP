...
- 核心适配器：`RedisAdapter`（CJS 输出）
  - 方法：`set(key, value, ttlSeconds?)`、`get<T>(key)`、`del(key)`、`exists(key)`、`ttl(key)`、`incr(key)`、`expire(key, seconds)`
...
- Nest 集成：`RedisModule.forRoot(...)` + `REDIS_KV` Token（CJS 输出）
  ...
  - 安全回退：未配置 `UPSTASH_REDIS_REST_URL/TOKEN` 时自动使用内存 KV（MemoryKV），保证应用可启动；配置就绪后自动切换为 Upstash
...
export class RateLimitInterceptor implements NestInterceptor {
  ...
  async intercept(ctx: ExecutionContext, next: CallHandler) {
    ...
    const key = `backend:ratelimit:${req.ip}:${req.method}:${req.url}`;
    const n = await this.kv.incr(key);
    if (n === 1) await this.kv.expire(key, Math.floor(this.windowMs / 1000));
    if (n > this.max) throw new HttpException('Too Many Requests', 429);
    return next.handle();
  }
}
```
...
- 建议 Node 版本使用 20+（supabase-js 官方对 Node 18 已提示弃用）
