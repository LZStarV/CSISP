...
- Postgres 由 Supabase 管理（参见 `/supabase` 与“环境搭建”）；Mongo 可通过 Docker 单独启动（端口 27017；健康检查 `mongosh ping`）
- Development 环境可与 Redis 一同本地运行：`redis + mongo`（Postgres 走 Supabase）
