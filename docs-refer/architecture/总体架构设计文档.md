...
| PostgreSQL (Supabase)       | 关系型数据库  | 由 Supabase 托管迁移、RLS 与 RPC          |
| Redis                       | 缓存/会话存储 | 提高热点数据访问速度，降低数据库压力      |
| supabase-js + supabase-sdk  | 数据访问门面  | 统一 RLS 用户态读取与 SECURITY RPC 写入   |
...
| 工具/配置      | 用途       | 说明                                                                  |
| -------------- | ---------- | --------------------------------------------------------------------- |
| Supabase CLI   | 数据库管理 | `/supabase` 目录集中迁移；本地使用 link/pull/reset；CI 执行 migration |
| Docker         | 可选依赖   | 本地 Redis/Mongo 单独拉起（如需）                                     |
