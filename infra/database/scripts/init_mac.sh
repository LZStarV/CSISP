...
if [ -z "${DB_HOST:-}" ]; then
  echo -e "\x1b[31m[ERROR] 未检测到必要环境变量（例如 DB_HOST）。请先运行 'infisical login' 并通过 'infisical run' 启动。\x1b[0m"
  exit 1
fi
...
docker compose -f "$ROOT_DIR/infra/database/docker-compose.db.yml" exec -T postgres psql -U "$POSTGRES_USER" -c "CREATE DATABASE csisp_dev OWNER $DB_USER;" >/dev/null 2>&1 || true
...
docker compose -f "$ROOT_DIR/infra/database/docker-compose.db.yml" exec -T postgres psql -U "$POSTGRES_USER" -d "csisp_dev" -c "GRANT ALL ON SCHEMA public TO $DB_USER;" >/dev/null 2>&1 || true
