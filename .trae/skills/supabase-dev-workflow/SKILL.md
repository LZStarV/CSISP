...
标准流程（本地 → Dev/Staging → Prod，在 supabase 目录下执行）
...
- 查看/修复命令（在 `/supabase` 且已 link）：
  - `pnpm run migrations:list`：已封装好，直接运行
  - 如需手动执行（如修复）：
    - `infisical run --env=staging -- sh -lc 'supabase migration repair --status reverted <ID> --db-url "$DATABASE_URL"'`
    - `infisical run --env=staging -- sh -lc 'supabase migration repair --status applied <ID> --db-url "$DATABASE_URL"'`
