...
- **严禁直接修改 `supabase/migrations/` 目录下已生成的迁移文件**（除非用户明确允许）
- 迁移文件由 `supabase db diff` 自动生成，代表数据库的真实状态快照，手动修改会导致本地与远程状态不一致
- 如果迁移文件内容不符合预期，应通过重新执行 `db diff` 或在远程数据库补充变更后重新生成来解决
